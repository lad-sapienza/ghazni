...
- **A build talks to the live BraDypUS server a lot** (one request per taxonomy category, plus one per record touched — several thousand requests for a full build). This is deliberate and was a conscious trade-off (see "Why build-time, not live" below) — but it does mean builds aren't instant (several minutes) and shouldn't be run in a tight loop.
- **Images are downloaded once per build**, into `public/images/bdus/` (gitignored — regenerated by `npm run fetch-images`, which `npm run build` runs first). Already-downloaded images are skipped on the next run, so a routine rebuild only fetches what's new.
...
The "finds" taxonomy (Alabaster, Marble, Dado panels, …) lives in **`src/data/finds-taxonomy.json`** — copied from the original PHP site's `tmpldata.json` (only renamed; its structure is unchanged). Each node is:

```json
"alabaster": {
  "id": "alabaster",
  "label": "Alabaster",
  "shortsql": "@finds~?archaeological_context|=|Islamic||and|main_material|=|Alabaster~>inv_no:asc"
}
```

- A node with `"items": { ... }` is a category with subcategories (renders as a tile grid).
- A node with `"shortsql": "..."` instead is a leaf (renders a grid of matching finds).
- `shortsql` is BraDypUS v4's old mini query language — kept as-is rather than translated, since `src/utils/shortsql.ts` translates it to BraDypUS v5's filter syntax automatically at build time. **To add a new category, add a new node using that same syntax** (copy a sibling and adjust the field/value) — you don't need to touch any component code.
...
**A whole new dataset/section** (something that isn't "finds" at all) is a bigger job than editing finds-taxonomy.json — it means new components and routes, not just new data. Start by reading `src/utils/bdus.ts` (the BraDypUS v5 client — auth, pagination, the ShortSQL translator) and `src/components/finds/` (how a taxonomy category page and a record page are built from it) as the reference pattern, and `src/pages/[domain]/finds/[...path].astro` for how routes are generated at build time via `getStaticPaths`. This client and pattern were written to be generalizable — if a second BraDypUS-backed site is built after this one, this is the code to extract into `@lad-sapienza/scms-core` as an official "BraDypUS source" (parallel to the existing Directus one), instead of rewriting it from scratch.
...
src/
  ...
  utils/
    ...
    shortsql.ts               # ShortSQL -> v5 filter translator
